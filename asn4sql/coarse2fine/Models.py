...
from .Utils import aeq, sort_for_pack
from . import modules
...
def _build_rnn(rnn_type, input_size, hidden_size, num_layers, dropout, weight_dropout, bidirectional=False):
    ...
    if weight_dropout > 0:
        ...
        rnn = modules.WeightDrop(rnn, param_list, dropout=weight_dropout)
    return rnn
...
class RNNEncoder(nn.Module):
    ...
    def __init__(self, rnn_type, bidirectional, num_layers,
                 hidden_size, dropout, lock_dropout, weight_dropout,
                 embeddings,
                 ent_embedding):
...
class ParserModel(nn.Module):
    def __init__(self, q_encoder, tbl_encoder, co_attention, agg_classifier, sel_match, lay_classifier, cond_embedding, lay_encoder, cond_decoder, cond_col_match, cond_span_l_match, cond_span_r_match, pad_word_index, split_word_index):
        ...
        self.span_merge = nn.Sequential(
            nn.Linear(2 * flags.FLAGS.rnn_size, flags.FLAGS.rnn_size),
            nn.Tanh())
        ...
        self.split_word_index = split_word_index
